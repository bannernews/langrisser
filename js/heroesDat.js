...
//	sd.innerHTML = '<table><tr><th>Имя героя</th><th>Бонд для ОЗ,ЗАЩ</th><th>Бонд для АТК/ИНТ</th><th>Нужен для бондов другим героям</th></tr>';
	...
	for (i=1;i<heroesDat.length;i++){
		...
		if(heroesDat[i][0]=='Трансцендентный' || heroesDat[i][0]=='Суверен ледяной бездны'){f = 'font-size:11px;'}else if(heroesDat[i][0]=='Капитан Коммандо и Гений-малыш' || heroesDat[i][0]=='Птица с поломанным крылом'){f = 'font-size:9px;'}else{f = 'font-size:12px;'}
