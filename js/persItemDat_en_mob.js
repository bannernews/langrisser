...
function persItemView (itemType) {
	...
	for (i=1;i<persItemDat.length;i++){
		if (persItemDat[i][4]==itemType){
			...
			hreff = ('hero_en_mob.html?name='+ persItemDat[i][0]).replace(/ /g,"%20");
			...
			'.png" height="70px" /><a href="'+hreff+'"><img style="position:absolute;top:43px;right:19px;" title="' + persItemDat[i][1] + '" src="images/head_circle/' + persItemDat[i][0] + '.png" height="40px"  /></a></div><div style="display:block;width:310px;float:left;margin-left:90px;"><b style="font-size: 18px;margin-right: 100%;float:left;">' + persItemDat[i][3]+'</b>' +
...
//			'</span> <img src="images/icon.png" height="25px" />
			...
			'</div></div><br><br><div style="white-space: normal;margin-top:44px;">' + descr + persItemDat[i][12]  + 
			'</div></div>';
		}
	}
}
